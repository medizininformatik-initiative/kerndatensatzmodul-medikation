...
3. **Fill the placeholders.** Open `sushi-config.yaml` and replace every `{{…}}`
   ...
   - `medikation` — lowercase short name (`person`), drives packageId/id/canonical.
   - `MII_IG_Medikation` — CamelCase (`Person`) → `name: MII_IG_Person`.
   - `MII IG Medikation` — the human-readable title part (`Person`), which becomes
...
4. **Rename the three placeholder-NAMED files.** Three file names carry a
   ...
   git mv 'input/pagecontent/ImplementationGuide-mii-ig-medikation.md' \
          'input/pagecontent/ImplementationGuide-mii-ig-person.md'
   git mv 'input/translations/de/pagecontent/ImplementationGuide-mii-ig-medikation.md' \
