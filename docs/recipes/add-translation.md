...
```text
input/translations/de/ImplementationGuide-mii-ig-medikation.po
```

**The publisher matches the catalogue to its IG resource BY FILE NAME**
(`<resourceType>-<id>`), so when you replace `medikation` in the file
*contents* you must **also rename the file** to match your IG's `id` in
`sushi-config.yaml`:

```sh
# your sushi-config.yaml has:  id: mii-ig-person
git mv 'input/translations/de/ImplementationGuide-mii-ig-medikation.po' \
...
> ImplementationGuide intro pages (`ImplementationGuide-mii-ig-medikation.md`
...
| **Page content is German but the title, breadcrumb and ToC entry stay English** | The IG-level catalogue was never renamed from `ImplementationGuide-mii-ig-medikation.po` to your IG id, so the publisher never matched it to the resource | Rename it to `ImplementationGuide-<your-ig-id>.po` (§5) |
