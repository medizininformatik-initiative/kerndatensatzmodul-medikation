...
| **Placeholder (`{{…}}`)** | A slot in the scaffold (e.g. `medikation`) you replace when creating a module. The CI self-check substitutes demo values so the template repo itself still builds. |
